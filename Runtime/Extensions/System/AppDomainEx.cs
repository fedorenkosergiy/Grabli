...
namespace System
{
    ...
    {
        private static Dictionary<AppDomain, List<Assembly>>
            allAssemblies = new Dictionary<AppDomain, List<Assembly>>();
        ...
        public static bool TryGetType(this AppDomain domain, string fullName, out Type type)
        {
            List<Assembly> assemblies = domain.GetAllAssemblies();
            for (int i = 0; i < assemblies.Count; ++i)
            {
                if (assemblies[i].TryGetType(fullName, out type))
                {
                    return true;
                }
            }
